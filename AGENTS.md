...
- 提交正文的 `- ` 分点列表**连续排列、项与项之间不要插入空行**；保持紧凑，仅在标题与正文、正文与 `Co-Authored-By` 尾注之间各保留一个空行。
...
- 提交者本人（如 Hermes/风澪）作为 author 提交时，不要追加自己的 `Co-Authored-By`；`Co-Authored-By` 仅用于实际参与的外部协作者（如 Codex）。
