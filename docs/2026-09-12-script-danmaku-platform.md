...
### 待办：图层时间窗（inPoint / outPoint）未生效（2026-09-24，定位中）

把窗口放回 333s 重录后，逐秒比对暴露的**新差异**（尚未修复）：

- **量测**（真 Chromium + 11 条真脚本）：121.0s 画布亮像素占比 1.0000（满亮）→ 122.2s 之后恒定 0.526，
  141.5s 之后 0.4125；原录屏 122s 是 0.85，**123s 之后只剩 0.008~0.02**。
- **视觉**（124/128/132/136/138/140/142/143s 对照）：我们画面里一直挂着一大块**浅灰蓝实心竖带**，
  原录屏同刻是黑底 + 细彩线/细字。
- 即：原作在约 122.5s 把「亮场景那一层」整个撤掉，我们把它留在了画布上。
- **定位线索**：宿主里 `inPoint` / `outPoint` / `layers` / `compWidth` 出现 **0 次**，而主脚本里分别是
  206 / 95 / 75 / 19 次——图层时间信息全在脚本与 Akari 库（entry_08）里。下一步查 Akari 是逐元素设 `lifeTime`，
  还是调宿主的元素生命周期 API，再决定补宿主还是补 lifeTime 解析。
