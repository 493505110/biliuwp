...
| **5. 打磨** | 性能（保留模式脏重绘/图层缓存/活跃剔除/节流）、可见性跟随 `LoadDanmu`、错误提示、懒初始化 | 综合体验 | 未开始（懒初始化已提前落地；保留模式脏重绘与离屏缓存已随阶段 1 落地，见 §阶段 1 实施记录 8；擦除/重合成收尾未收敛，见上方状态行） |
...
- **原舞台就是视频画面区**：作品舞台等于视频画面区（16:9 显示区域），绝对像素随显示尺寸走——
  原录屏上传的是 1920×1080，B站默认只下发 360P（640×360）流，两者是同一画面按 3:1 等比缩放。
- **1:1 对齐证据**：把探针画布设成 640×360（与 360P 流画面同尺寸）后，探针渲染结果与原录屏第 115s 的帧
  1:1 对齐，缩放 1.0、平移 (0, 0)、NCC 0.868；1080P 流缩到 640×360 后与 360P 流同水平（0.795）。
  画布尺寸一旦不等于视频渲染矩形，缩放与平移都会偏离这个基准。
...
保真核对时挖到的真差异：条目结束后，宿主会在画布上留一块灰蓝残影，一直不散，
而同一时刻的原录屏画面里没有对应内容。

（**注意**：本节当时用来触发这个工况的「条目窗口 = 122s」是错的，见下文「更正」一节；
但条目窗口真的结束时必须清掉该条目像素这一条结论仍然成立。）
...
### 更正：条目时间窗不是 122.5s，尾部也不是「整层消失」（2026-09-24）

上一版把条目窗口取成 `duration=122.5s`，理由是「原录屏里弹幕层在 ~122s 整层消失」。**这个前提是错的**，已更正：

- **弹幕数据（官方接口）**：`x/v2/dm/web/view?type=1&oid=4369799&pid=2669196` 返回 1 个特殊弹幕包
  （`i0.hdslb.com/bfs/dm/6cadf9a33a4db9c861b5cdd31e3224852bfcc51d.bin`，796,313 字节），解出
  **11 条 mode=8 elem**（id 1147131631~1147131639、1147140491 = Akari 库、1147217315 = 主脚本；
  progress 0.00s×9 / 0.10s / 0.20s），与仓库 fixtures 一一对应且 `entry_10_1147217315.js` 与 elem 同为 353,975 字符。
  同一作品的 XML 池是 **600 条**（599 滚动 + 1 顶部，**不含 mode 8**）；
  留档录屏 BV1c7411H7jF（cid=148436020）自己的池是**空的**，所以录屏画面里的内容全部来自原作播放器。
- **关键证据**：录屏 124s/128s 那些细文字（「つぎのきょうは」「在下一个今天」「かぎりつけるんだ」「装点上就行了」
  「しらないみ」）**都在主脚本 id=1147217315 里**。所以 122.5s 之后不是弹幕层结束，而是**换场景**：
  亮场景层退出、细线 + 歌词层接上，两层都是代码弹幕。
- 因此正确的时间窗是组合长度（333s），不是 122.5s；按 122.5s 录出来的视频，尾部 20 秒是被错误截断的
  （窗口放回 333s 重录即可恢复）。
- **仍然成立**：条目窗口真的结束（例如播放头离开窗口）时，宿主必须清掉该条目的像素——即上一节的摘除路径修正
  与画布收口，仍由 D23/D25/D26 守住。

### 图层隐藏 / 淡出的残影修正与残留（2026-09-24）

**机制（实证）**：Akari 的图层切换不是删元素，而是开关 `visible`——`Akari.Display.Layer.update(time)` 就是
`time < inPoint || time >= outPoint ? this.source.visible = false : this.source.visible = true`；
库里所有显示对象都用 `lifeTime: 810114514` 哨兵创建，元素永不删除。所以「图层结束」在宿主里表现为
一次属性写入，而不是摘除。

本轮补掉的两个缺口（都在同一个坑上）：

1. **`visible = false` 对嵌套元件擦不到**：`markElementHidden()` 只处理「自己有主画布矩形」的元件，
   而嵌套元件（Akari 的图层 canvas 正是嵌套 sprite）`lastPaintedRect` 为 null，`enqueueElementErase`
   一个矩形都入不了队——与 D23 那次完全同形。修法同前：自己没有矩形时向上找最近一个画到主画布上的
   祖先，退掉它的旧矩形。
2. **`alpha` 归零没有走隐藏路径**：真作品 Comp0 的「背景2」层是整幅 1280×720 的矩形 + 关键帧 alpha
   （`inPoint: 2700, outPoint: 3750`），淡出后旧像素留在画布上，形成整块残影。修法：alpha 归零等同隐藏。

**验证**：
- 行为用例 D27（visible）/ D28（alpha）在修复前的宿主上双双 FAIL、修复后通过；`retained-mode` 28/28、
  `real-m8-scripts` 5/5、`dotnet test` 241/241。
- 真 Chromium 端到端（窗口 333s）：124s 之后画布亮像素占比由 **0.505 降到 0.0945**，
  140s 之后由 **0.4125 降到 0.0183**；原录屏 123s 之后是 0.008~0.022 —— 后半段已落进原录屏区间。

**残留（未修）**：124~134s 仍有 0.078~0.095（原录屏同刻 0.011~0.022）。怀疑是「连续 alpha 变化时
只合成、不清旧像素」的叠加效应（等价于 damage-rect 级问题），需要按「先擦上一帧该元件落笔区域、
再合成」的模型处理，不能简单擦祖先矩形（那条路已在 D24 被证明会打空洞）。
