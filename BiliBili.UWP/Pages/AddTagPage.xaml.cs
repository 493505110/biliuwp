    ...
    public sealed partial class AddTagPage : Page
    {
        ...
        private async void GetLikeTag()
        {
            try
            {
                gv_like.Items.Clear();
                pr_Load.Visibility = Visibility.Visible;
                string url = string.Format("https://app.bilibili.com/x/feed/subscribe/tags?access_key={0}&appkey={1}&build=434300&mobi_app=android&platform=wp&pn=1&ps=60&ts={2}000",ApiHelper.access_key,ApiHelper.AndroidKey.Appkey,ApiHelper.GetTimeSpan);
