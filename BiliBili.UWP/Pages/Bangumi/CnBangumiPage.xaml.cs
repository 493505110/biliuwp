...
namespace BiliBili.UWP.Pages
{
    ...
    public sealed partial class CnBangumiPage : Page
    {
        ...
        private async void LoadHome()
        {
            ...
            {
                LogHelper.WriteLog("读取国漫页面信息失败", LogType.ERROR, ex);
                ...
                {
                    Utils.ShowMessageToast("读取国漫信息失败", 3000);
                }
            }
            finally
            {
        ...
        private async Task<List<SeasonIndexResultItemModel>> LoadSeasonList(SeasonIndexAPI seasonApi, int isFinish)
        {
            ...
            if (data == null)
            {
                throw new InvalidOperationException("国漫信息解析失败");
            }
            if (data["code"].ToInt32() != 0)
            {
                throw new InvalidOperationException(data["message"]?.ToString() ?? "国漫信息加载失败");
            }

            var list = data["data"]?["list"];
            if (list == null)
            {
                throw new InvalidOperationException("国漫信息为空");
            }
